...
flags.DEFINE_integer(name='normal_class', default=0, help='')
...
flags.DEFINE_string(name='kernel', default='rbf', help='')
...
FLAGS = flags.FLAGS
log = Logger().getLogger("__anomaly_detection.py__")
...
def main():
    ...
    model.load_state_dict(torch.load('runs/Apr07_12-47-12_admin4/resnet50_checkpoint_0200.pth.tar')['state_dict'])
    model.eval()
    # init OCSVM model
    ocsvm = OCSVM(params, model)

    # prepare dataset for OCSVM(feature extraction, split...)
    dataset = ADDataset(normal_classes=params['normal_class'])
