...
@click.option('--lr_milestone', type=int, multiple=True,
...
@click.option('--ae_lr_milestone', type=int, multiple=True,
              help='Lr scheduler milestones at which lr is multiplied by 0.1. Can be multiple and must be increasing.')
...
def main(dataset_name, net_name, xp_path, data_path, load_config, load_model, objective, nu, device, seed,
    ...
    # Load encoder
    model = Simclr('resnet50', num_classes=10, is_pretrained=False)
    model.load_state_dict(torch.load('../../runs/Apr07_12-47-12_admin4/resnet50_200_best.pth.tar')['state_dict'])
    model.eval()
